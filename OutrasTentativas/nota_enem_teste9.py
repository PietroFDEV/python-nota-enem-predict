...
# Parâmetros para ajustar (XGBoost)
parametros_xgb = {
...
# Parâmetros para ajustar (LightGBM)
parametros_lgbm = {
    'n_estimators': [100, 200, 300],
    'learning_rate': [0.01, 0.05, 0.1],
    'max_depth': [3, 5, 7],
...
grid_search_xgb = GridSearchCV(XGBRegressor(random_state=42), parametros_xgb, cv=5, scoring='neg_mean_squared_error')
...
grid_search_lgbm = GridSearchCV(LGBMRegressor(random_state=42), parametros_lgbm, cv=5, scoring='neg_mean_squared_error')
